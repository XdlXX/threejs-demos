...
import { getRotateMatrix, getScaleMatrix, getTranslateMatrix, maxMatrix } from '~/utils'
...
function initWebGL() {
  ...
  const animate = () => {
    ...
    if (x > 1)
      x = 0
    const translate = getTranslateMatrix(x, x)
    const scale = getScaleMatrix(x, x)
    const rotate = getRotateMatrix(x * Math.PI * 2)
    const matrix = maxMatrix(maxMatrix(translate, scale), rotate)
